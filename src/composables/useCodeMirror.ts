...
/** `CodeMirror` component props definition. */
export const codeMirrorProps = {
  /** Model value */
  modelValue: {
...
/** Resolved prop types for {@link codeMirrorProps}. */
export type CodeMirrorProps = ExtractPropTypes<typeof codeMirrorProps>;

/** `CodeMirror` component emits definition. */
export const codeMirrorEmits = {
  /** Model Update */
...
/** Typed `emit` function matching {@link codeMirrorEmits}. */
export type CodeMirrorEmit = {
  (event: 'update:modelValue', value: string | Text): void;
  (event: 'update', value: ViewUpdate): void;
  (
    event: 'ready',
    value: {
      view: EditorView;
  ...
  (event: 'change', value: EditorState): void;
  (event: 'destroy'): void;
};

/**
 * Sets up a CodeMirror 6 editor instance bound to `props` and reports
 * changes back through `emit`.
 *
 * This encapsulates all editor lifecycle, reactivity, and command logic so
 * it can be reused outside of the `CodeMirror` component (e.g. in a custom
 * render function or another component).
 *
 * @param props - Resolved `CodeMirror` component props.
 * @param emit - Typed emit function.
 */
export function useCodeMirror(props: CodeMirrorProps, emit: CodeMirrorEmit) {
  /** Editor DOM */
   ...
   */
  const selection: WritableComputedRef<EditorSelection | undefined> = computed({
